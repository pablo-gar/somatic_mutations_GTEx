...
#output <- "/scratch/users/paedugar/somaticMutationsProject/generalMutationAnalyses/results/mutation_expression_associations_targeted_expressionMat/Colon_Transverse/n6_0.0_0.7/expression_mat.txt"
...
rownames(pathway_gene_table) <- pathway_gene_table$gene
...
expressionMat <- expressionMat[targetGenes_ind,]
...
# Write expression mat
expressionMat$gene <- pathway_gene_table[rownames(expressionMat), 'alias']
write.table(expressionMat, output, sep = "\t", quote = F, col.names = T, row.names = T)
