...
dndscv = function(mutations, gene_list = NULL, refdb = "hg19", sm = "192r_3w", kc = "cgc81", cv = "hg19", max_muts_per_gene_per_sample = 3, max_coding_muts_per_sample = 3000, use_indel_sites = T, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = T, outp = 3, filter_false_pos = NULL) {
    ...
    # Eliminating mutations that may be false-positives (i.e. the ones comprising most of a gene's mutations
    if(!is.null(filter_false_pos)) {
        library("dplyr")
        # Helper function
        find_false_pos <- function(mutations, cutoff) {
            ...
            return(indices_to_eliminate)
        }
        
        # Eliminating false positives
        browser()
        to_eliminate <- find_false_pos(mutations, cutoff = filter_false_pos) 
        mutations <- mutations[!to_eliminate,]
    }
    ...
    for (j in 1:nrow(mutations)) {
        ...
        if (any(pos == RefCDS[[geneind]]$intervals_splice)) { # Essential splice-site substitution
            ...
            new_codon = old_codon; new_codon[pos_in_codon] = as.character(mutations$mut_cod[j])
    ...
    if (any(!is.na(wrong_ref))) {
        ...
        warning(sprintf('%0.0f mutations have a wrong reference base, please correct and rerun.',sum(!is.na(wrong_ref)))) # This can be made into a mere warning and the rest of the code will work
