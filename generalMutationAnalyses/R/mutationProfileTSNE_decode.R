...
color_point <- rbind(
                     ...
                     data.frame(a = "mother", b = "#f936ff"),
                     data.frame(a = "father", b = "#2525ff"),
...
#workingDir <- "/scratch/users/paedugar/somaticMutationsProject/mutationCount_decode/mutationMatrix"
#Wfiles2 <- list.files(workingDir)[grep(".*n6.*.txt", list.files(workingDir))]
#Wfiles2 <- file.path(workingDir, Wfiles2)
#Wfiles <- c(Wfiles, Wfiles2)
#contextLength <- 5
#tsneOutputPrefix <- "~/n6"
...
plotColoredScatterFromMatrix <- function(x, categories = NULL, col = tissueCol, extraCol = NULL, alpha = NULL) {
    ...
    if(is.null(extraCol)) {
        ...
        if(is.character(extraCol))
            extracol <- as.numeric(as.factor(extraCol))
        if(is.factor(extraCol))
            extraCol <- as.numeric(extraCol)
        x$col <- extraCol
    }
...
doSaveTSNE <- function(Wjoint, doPCA, minVarExplained, tsneOutputPrefix, useFreq = T, do_extended_plots = F, alphas = NULL, tissues_sil = NULL) {
    ...
    if(do_extended_plots) {
        ...
        #tsne_meta <- read_all_metadata(sraIds, readCountFile, transDiversityFile, sraInfo = c(`molecular_data_type_s` = 24), na.rm = F)
        #tsne_meta$n_uniqueMapped <- log10(tsne_meta$n_uniqueMapped)
...
# Only samples with enough mutations
Wjoint <- Wjoint[,colSums(Wjoint) > 0]

sraIds <- gsub(".+\\|(.+)", "\\1", colnames(Wjoint))

sra_indeces <- grep("SRR", sraIds)


# Normalize by freqeuncy of nucleotides in entire transcriptome
if(normilizeByExpression) {
    geneExp <- readGeneExpression(sraIds[sra_indeces])
    contextCounts <- normalizeByExpression(Wjoint[,sra_indeces], oligoCountFile, geneExp)
}
...
if(normilizeByExpression) {
    ...
    # Do percentage per row
    Wjoint_copy <- Wjoint_copy / matrix(rep(colSums(Wjoint_copy), each = nrow(Wjoint_copy)), byrow = F, nrow =  nrow(Wjoint_copy), ncol = ncol(Wjoint_copy))
    doSaveTSNE(Wjoint = Wjoint_copy, doPCA = doPCA, useFreq = F, minVarExplained = VAR_EXPLAINED_PCA, tsneOutputPrefix = paste0(tsneOutputPrefix, "_normalized_"), 
               do_extended_plots = T, alphas = c("mother", "father", "Ovary", "Prostate"))
    #doSaveTSNE(Wjoint = Wjoint_copy[,sra_indeces], doPCA = doPCA, useFreq = F, minVarExplained = VAR_EXPLAINED_PCA, tsneOutputPrefix = paste0(tsneOutputPrefix, "_normalized_"), 
    #           do_extended_plots = F, alphas = c("mother", "father", "Ovary", "Prostate"))
    
    #doSaveTSNE(Wjoint = Wjoint_copy, doPCA = doPCA, useFreq = F, minVarExplained = VAR_EXPLAINED_PCA, tsneOutputPrefix = paste0(tsneOutputPrefix, "_normalized_"), 
    #           do_extended_plots = T, alphas = c("Skin", "Spleen", "Lung", "Liver", "Esophagus_Muco", "Whole_Blood", "Thyroid", "Artery_Coronary", "Colon_T|Small", "Muscle|Heart"),
    #           tissues_sil = c("Colon_T|Small", "Brain", "Skin", "Muscle|Heart"))
