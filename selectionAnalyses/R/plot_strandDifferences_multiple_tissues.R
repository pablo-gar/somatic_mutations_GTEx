...
mutationFiles <- args[-(1:4)] 

#outPrefix <- c("/scratch/users/paedugar/somaticMutationsProject/selection/strand_differences_pairwise_comparisons/n6_0.0_0.7/")
#out_table_gwas_folder <- "/scratch/users/paedugar/somaticMutationsProject/mutationCount/strand_fc/n6_0.0_0.7/"
...
write_table_for_GWAS <- function(mutationStats_all_sraId, readCountFile, transDiversityFile, out_table_gwas_folder) {
    ...
    for(currentTissue in unique(mutationStats_all_sraId$tissue)) {
        ...
        # Working with total values
        current_total <- mutationStats_all_sraId %>%
            filter(tissue ==  currentTissue) %>%
            mutate(total = `TRUE` + `FALSE`) %>%
            select(sampleID, tissue, mutation, total) %>%
            spread(mutation, total) %>% 
            dplyr::rename(sample = sampleID) %>%
            select(-tissue)
        
        current_total <- current_total[,ncol(current_total):1]
        current_total <- as.data.frame(current_total)
        rownames(current_total) <- current_total$sample
        
        write.table(current, file.path(out_table_gwas_folder, paste0(currentTissue, "-fcSrands_allInds_noMetadata.txt")), sep  = "\t", quote = F, col.names = T, row.names = F)
        write.table(current_total, file.path(out_table_gwas_folder, paste0(currentTissue, "-fcSrands_allInds_noMetadata_total_mutations.txt")), sep  = "\t", quote = F, col.names = T, row.names = F)
        ...
        if(!grepl("EXO", currentTissue)) {
            ...
            for(mut_type in c("C_A", "C_G", "C_T", "T_A", "T_C", "T_G")) {
                for(feature in c("AGE", "GENDER", "BMI")) {
                    cor_results <- cor.test(current[,mut_type], current[,feature], na.action = na.exclude)
                    metadata_cors[[counter]] <- data.frame(mut = mut_type, feature = feature, tissue = currentTissue, spearman = cor_results[["estimate"]], p_value = cor_results[["p.value"]])
                    counter <- counter + 1 
                }
            }
        }
    ...
    metadata_cors <- do.call(rbind, metadata_cors)
    metadata_cors <- metadata_cors %>%
        group_by(feature) %>%
        mutate(fdr_feature = p.adjust(p_value, method = "BH")) %>%
        ungroup()
    metadata_cors <- metadata_cors[order(metadata_cors$fdr_feature),]
    
    write.table(metadata_cors, file.path(outPrefix, paste0("1_metadata_fcSrands_cors.txt")), sep  = "\t", quote = F, col.names = T, row.names = F)
