...
main <- function(cmdArgs = commandArgs(T)) {
    ...
    #tissue <- "Lung"
    #phenotype_var <- "vaf"
    #outputSignificant <- file.path("/scratch/users/paedugar/somaticMutationsProject/vaf/expression_association_clonality/", phenotype_var, tissue, "significant.txt.gz")
    #outputNon <- file.path("/scratch/users/paedugar/somaticMutationsProject/vaf/expression_association_clonality", phenotype_var, tissue, "nonSignificant.txt.gz")
    #outputPlots <- file.path("/scratch/users/paedugar/somaticMutationsProject/vaf/expression_association_clonality/", phenotype_var, tissue, "plots")
    #outputGO <- file.path("/scratch/users/paedugar/somaticMutationsProject/vaf/expression_association_clonality/", phenotype_var, tissue, "GO_results.txt")
    ...
    expressionMat[expressionMat == 0] <- NA
    ...
    for(i in MUT_TYPES[MUT_TYPES %in% colnames(mutation_map)]) {
        ...
        for(k in 1:nrow(significant)) {
            ...
            p <- scatter(data.frame(y = phenoVectNorm, x = current_exp), x = "x", y = "y", regression =T, method='pearson') + theme_sleek()
